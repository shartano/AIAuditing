...
def main():
    ...
    if args.write_observations:
        obs_dirname = os.path.dirname(args.write_observations)
        if obs_dirname:
            os.makedirs(obs_dirname, exist_ok=True)
    ...
    # NOTE: Your scene schema may include scale fields with null values (pre-scale). That is OK.
    prev_scale = scene.get("scale", {})
    if not isinstance(prev_scale, dict):
        prev_scale = {}

    prev_mpcu = prev_scale.get("meters_per_colmap_unit", None)
    already_scaled = isinstance(prev_mpcu, (int, float)) and math.isfinite(float(prev_mpcu)) and float(prev_mpcu) > 0
    already_in_meters = (scene.get("units") == "world_meters")

    if already_scaled or already_in_meters:
        print("[Step4] WARNING: scene appears to already be in meters (possible double-scale risk).")
        if args.fail_on_warning:
            raise RuntimeError("Scene already appears to be scaled. Aborting.")

    # Inject scale metadata (copy to avoid mutating prev_scale before the guard above)
    scene_meta = dict(prev_scale)
    scene_meta.update({
        "method": "aruco_triangulated_corners_colmap_poses",
    ...
    # After scaling, the world is expressed in meters
    scene_scaled["units"] = "world_meters"
    out_dirname = os.path.dirname(args.scene_out)
    if out_dirname:
    ...
    diag_dirname = os.path.dirname(args.diag_out)
    if diag_dirname:
        os.makedirs(diag_dirname, exist_ok=True)
