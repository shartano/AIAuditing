...
def main():
    ...
    ap.add_argument("--cluster_dist_toilet", type=float, default=0.75, help="Clustering distance (aligned units) for toilet instances")
    ...
    ap.add_argument("--toilet_max_p95_y", type=float, default=3.0,
    ...
    # Load scene (planes were produced by step2 in aligned frame) :contentReference[oaicite:3]{index=3}
    ...
    if not detections_by_image:
        print("[step3] No detections after filtering.", file=sys.stderr)
        return 0
    ...
    if not candidates:
        print("[step3] All detections were filtered out (no 3D support).", file=sys.stderr)
        return 0
    ...
    def add_instance(cls: str, idxs: List[int]):
        ...
        if cls == "toilet":
            if len(mem) < args.min_detections_toilet:
                return
